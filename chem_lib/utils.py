...
def init_trial_path(args,is_save=True):
    ...
    """
    datasetname = args.d_names

    prename = datasetname + '_' + str(args.test_dataset) +  '_' + args.enc_gnn + \
              '_k' + str(args.n_shot_train) + '_q' + str(args.n_query) + '_lr' + str(args.meta_lr) + '_in' + str(args.inner_lr) + \
              '_ste' + str(args.update_step_test) + '_stein'+ str(args.inner_update_step)+ '_pad' + str(args.pad) + '_ep' + str(args.epochs) \
              + '_seed' + str(args.seed) + '_maml' + str(args.maml) + '_pi' + str(args.pi) \
              + '_alr' + str(args.agentlr)
...
def save_args(args):
    ...
    prename=f"upt{args['update_step_test']}-{args['inner_lr']}_mod{args['batch_norm']}-{args['rel_node_concat']}"
    prename+=f"-{args['rel_hidden_dim']}-{args['rel_res']}"
    prename+=f"-{args['meta_lr']}-{args['d_topk']}-{args['d_ploss']}"
