...
def obtain_train_test_tasks_ms(d, td, sourcenames = None):

    real_tox21 = tox21_tasks = list(range(12))
    real_sider = sider_tasks = list(range(27))
    ...
    toxcast_train_tasks.extend(toxcast_test_tasks)
    real_toxcast = toxcast_train_tasks
    #  modify from task index to task_id index
    toxcast_tasks = range(len(toxcast_train_tasks))
    real_muv = muv_tasks = list(range(0, 17))
    train_dict = {}
    name_dict = {'tox21': tox21_tasks, 'toxcast': toxcast_tasks, 'muv': muv_tasks, 'sider':sider_tasks}
    real_name_dict = {'tox21': real_tox21, 'toxcast': real_toxcast, 'muv': real_muv, 'sider':real_sider}

    if d == 'all':
        if sourcenames is None:
            msource = ['tox21', 'sider', 'toxcast', 'muv']
        else:
            msource = [str(item) for item in sourcenames.split('-')]

        if len(msource) > 1:
            msource.remove(td)

    for name in msource:
        train_dict[name] = name_dict[name]

    return train_dict, name_dict[td], real_name_dict
...
def obtain_distr_list(dataset):
    list_sider = [[684, 743], [431, 996], [1405, 22], [551, 876], [276, 1151], [430, 997], [129, 1298], [1176, 251],
    ...
    list_tox21 = [[6956, 309], [6521, 237], [5781, 768], [5521, 300], [5400, 793], [6605, 350], [6264, 186], [4890, 942],
                [6808, 264], [6095, 372], [4892, 918], [6351, 423]]
    list_muv = [[14814, 27], [14705, 29], [14698, 30], [14593, 30], [14873, 29], [14572, 29], [14614, 30], [14383, 28],
                [14807, 29], [14654, 28], [14662, 29], [14615, 29], [14637, 30], [14681, 30], [14622, 29], [14745, 29],
                [14722, 24]]
    list_toxcast = [[1288, 443], [1434, 297], [859, 175], [991, 43], [791, 243], [734, 300], [589, 445], [972, 62],
    ...
    if dataset == "sider":
        ...
        assert dataset == 'all'
        dataset_all = {}
        dataset_all['sider'] = list_sider
        dataset_all['tox21'] = list_tox21
        dataset_all['muv'] = list_muv
        dataset_all['toxcast'] = list_toxcast
...
def sample_equal_meta_datasets(data, dataset_name, task_id, real_task , n_shot, n_query):

    distri_list = obtain_distr_list(dataset_name)

    thresh = distri_list[real_task][0]
    neg_sample = sample_inds(range(0, thresh), n_shot)
    neg_rem = [i for i in range(0, thresh) if i not in neg_sample]
    qneg_sample = sample_inds(neg_rem, n_query)
    pos_sample = sample_inds(range(thresh, len(data)), n_shot)
    pos_rem = [i for i in range(thresh, len(data)) if i not in pos_sample]
    qpos_sample = sample_inds(pos_rem, n_query)

    s_list_1 = neg_sample[:n_shot] + pos_sample[:n_shot]
    q_list_1 = qneg_sample + qpos_sample
...
def sample_meta_datasets(data, dataset_name, task_id, real_task , n_shot, n_query):
...
def sample_test_datasets(data, dataset_name, task, real_task,  n_shot, n_query,  n_query_test, update_step=1):
    distri_list = obtain_distr_list(dataset_name)
    thresh = distri_list[real_task][0]
    ...
    if len(q_list) > n_query_test:
        q_test = sample_inds(q_list, n_query_test)
    else:
        q_test = q_list
    s_data = data[torch.tensor(s_list)]
    q_data = data[torch.tensor(q_test)]
